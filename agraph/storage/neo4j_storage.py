...
class Neo4jStorage(GraphStorage):
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "neo4j package not installed. Please install it with: pip install neo4j"
                )
                return False
            ...
            with self.driver.session(database=self.database) as session:
                ...
                graph = KnowledgeGraph(
                    ...
                    created_at=datetime.fromisoformat(
                        graph_metadata.get("created_at", datetime.now().isoformat())
                    ),
                    updated_at=datetime.fromisoformat(
                        graph_metadata.get("updated_at", datetime.now().isoformat())
                    ),
                )
    ...
    def _load_relations(
        self, session, graph_id: str, entities: Dict[str, Entity]
    ) -> List[Relation]:
        """加载关系"""
        result = session.run(
            """
    ...
    def _record_to_relation(
        self, record: Dict[str, Any], head_entity: Entity, tail_entity: Entity
    ) -> Relation:
        """将数据库记录转换为关系对象"""
        return Relation(
            id=record.get("id", ""),
