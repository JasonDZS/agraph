...
class BaseKnowledgeGraphBuilder(ABC):
    ...
    def validate_graph(self, graph: KnowledgeGraph) -> Dict[str, Any]:
        ...
        if cycles:
            validation_result["issues"].append(
                {"type": "cycles", "count": len(cycles), "cycles": cycles[:5]}  # 只显示前5个
            )
    ...
    def build_graph(
        ...
        try:
            ...
            implicit_relations = self.text_relation_extractor.infer_implicit_relations(
                list(graph.entities.values()), list(graph.relations.values())
            )
...
class MultiSourceGraphBuilder(StandardGraphBuilder):
    ...
    def build_graph_from_multiple_sources(
        self, sources: List[Dict[str, Any]], graph_name: str = "multi_source_graph"
    ) -> KnowledgeGraph:
        """
        从多个数据源构建知识图谱
