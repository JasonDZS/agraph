...
class TokenChunker:
    ...
    def split_text_with_positions(
        ...
        """
        Split text into chunks based on token count and return position information.
        ...
        Returns:
            List of tuples (chunk_text, start_index, end_index) where indices are character positions
        """
        # Split text without overlaps to get accurate positions
        original_overlap = self.chunk_overlap
        self.chunk_overlap = 0  # Temporarily disable overlap for position calculation

        try:
            regular_chunks = self.split_text(text, separators)
        finally:
            self.chunk_overlap = original_overlap  # Restore original overlap

        # Now find their positions in the original text
        chunks_with_positions = []
        search_start = 0

        for chunk in regular_chunks:
            # Find the chunk in the original text starting from search_start
            chunk_start = text.find(chunk, search_start)
            if chunk_start == -1:
                # Try searching from beginning if not found (may happen with stripped chunks)
                chunk_start = text.find(chunk.strip(), search_start)
                if chunk_start == -1:
