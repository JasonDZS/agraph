...
class HTMLProcessor(DocumentProcessor):
        ...
        try:
            ...
            for alt_encoding in ["latin-1", "cp1252", "iso-8859-1", "utf-16"]:
                try:
                    ...
                    continue
            raise ProcessingError(
                f"Could not decode HTML file {file_path} with any supported encoding"
            ) from None  # Multiple exceptions possible, no single source
    ...
    def _remove_non_content_elements(self, soup: "BeautifulSoup") -> None:
        ...
        # Remove comments
        from bs4 import Comment  # pylint: disable=import-outside-toplevel
    ...
    def _extract_structured_text(self, soup: "BeautifulSoup") -> str:
        ...
        text_parts.extend(self._extract_title(soup))
        text_parts.extend(self._extract_headings(soup))
        text_parts.extend(self._extract_paragraphs(soup))
        text_parts.extend(self._extract_lists(soup))
        text_parts.extend(self._extract_tables(soup))
        text_parts.extend(self._extract_remaining_content(soup))
    ...
    def _extract_title(self, soup: "BeautifulSoup") -> List[str]:
        ...
        Returns:
            List containing the title if found.
        """
    ...
    def _extract_lists(self, soup: "BeautifulSoup") -> List[str]:
        """Extract list items with bullets and numbers.
        ...
        # Extract unordered list items with bullets
    ...
    def _extract_remaining_content(self, soup: "BeautifulSoup") -> List[str]:
        """Extract any remaining content that wasn't captured by other methods.

        Args:
            soup: BeautifulSoup object.

        Returns:
            List of remaining text content.
        """
        remaining_content = []
        excluded_tags = {"title", "h1", "h2", "h3", "h4", "h5", "h6", "p", "li", "td", "th"}

        for element in soup.find_all(text=True):
            if (
                hasattr(element, "parent")
                and element.parent
                and hasattr(element.parent, "name")
                and element.parent.name not in excluded_tags
            ):
