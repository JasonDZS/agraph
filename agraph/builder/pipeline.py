...
class BuildPipeline:
    ...
    def get_pipeline_metrics(self) -> dict:
        ...
        if self._execution_metrics["total_executions"] > 0:
            success_rate = (
                self._execution_metrics["successful_executions"] / self._execution_metrics["total_executions"]
            ) * 100

        avg_execution_time = 0.0
        if self._execution_metrics["total_executions"] > 0:
            avg_execution_time = (
                self._execution_metrics["total_execution_time"] / self._execution_metrics["total_executions"]
            )

        return {
            "pipeline_info": {"total_steps": len(self.steps), "step_names": self.get_step_names()},
            "execution_metrics": {
