...
class DocumentProcessingStep(BuildStep):
    ...
    def __init__(self, document_processor: DocumentProcessor, cache_manager: Any):
        """
    ...
    async def _execute_step(self, context: BuildContext) -> StepResult[List[str]]:
        ...
        try:
            ...
            document_paths_typed = cast(List[Union[str, Path]], document_paths)
            texts = self.document_processor.process_documents(document_paths_typed, context.use_cache)
            ...
            for i, text in enumerate(texts):
                if not isinstance(text, str):
                    return StepResult.failure_result(f"Invalid text at index {i}: expected str, got {type(text)}")
