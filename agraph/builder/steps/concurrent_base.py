        ...
        try:
            ...
            if not items:
                return StepResult.success_result(
                    [],
                    metadata={"message": f"No items to process for {self.name}"}
                )
            ...
            acquired_resources = await self.concurrency_manager.acquire_resources(
                self.name, self._required_resources
            )
            ...
            try:
                ...
                if not self._validate_batch_results(results):
                    return StepResult.failure_result(
                        f"Step {self.name}: Batch processing validation failed"
                    )
            ...
            return StepResult.failure_result(
                f"Step {self.name}: Execution timeout",
                StepError.timeout_error(f"Step {self.name} exceeded timeout limit")
            )
        except Exception as e:
            logger.error(f"Step {self.name}: Execution failed with error: {str(e)}")
            return StepResult.failure_result(
                f"Step {self.name}: {str(e)}",
