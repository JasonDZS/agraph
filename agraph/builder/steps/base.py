...
class BuildStep(ABC):
    ...
    async def execute(self, context: "BuildContext") -> StepResult[Any]:
        ...
        try:
            ...
            if result.is_success():
                ...
                logger.error(f"Step {self.name}: Failed with error: {result.error.message}")
    ...
    def _should_use_cache(self, context: "BuildContext") -> bool:
        ...
        """
        return (
            context.use_cache and 
            not context.should_execute_step(self.name) and
            self._has_cached_result(context)
        )
    ...
    def _get_cached_result(self, context: "BuildContext") -> Any:
        ...
        return self.cache_manager.get_step_result(
            self.name,
            input_data, 
            expected_type
        )
