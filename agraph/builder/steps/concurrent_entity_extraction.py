...
class ConcurrentEntityExtractionStep(ConcurrentBuildStep[Entity], ParallelProcessingMixin):
    ...
    def _group_chunks_by_document(self, chunks: List[TextChunk]) -> dict:
        ...
        """
        doc_groups: Dict[str, List[TextChunk]] = {}
        for chunk in chunks:
    ...
    async def _process_document_batch(
        self, doc_id: str, doc_chunks: List[TextChunk], context: BuildContext
    ) -> List[Entity]:
        """
            ...
            enhanced_metadata.update(
                {
                    ...
                    "extraction_efficiency": (
                        len(entities) / enhanced_metadata.get("items_processed", 1)
                        if enhanced_metadata.get("items_processed", 0) > 0
                        else 0
                    ),
                }
            )
