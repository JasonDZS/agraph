...
class ConcurrencyConfig:
    ...
    def from_settings(cls) -> "ConcurrencyConfig":
        ...
        return cls(
            # Scale based on system capabilities - use defaults since system config doesn't exist
            text_chunking_workers=4,
            entity_extraction_workers=8,
            relation_extraction_workers=6,
            cluster_formation_workers=4,
...
class ConcurrencyManager:
    ...
    def __init__(self, config: Optional[ConcurrencyConfig] = None):
        ...
        self._resource_usage = {"memory_mb": 0, "active_llm_calls": 0, "active_documents": 0}
    ...
    def release_resources(self, step_name: str, acquired_resources: dict) -> None:
        """Release acquired resources."""
        for _, semaphore in acquired_resources.items():
            semaphore.release()
    ...
    def get_resource_stats(self) -> dict:
        ...
        return {
            ...
            "semaphore_availability": {
                name: sem._value for name, sem in self._semaphores.items()  # pylint: disable=protected-access
            },
        }

    async def batch_process(self, items: list, processor_func: Callable, batch_size: int, resource_type: str) -> list:
        """
        ...
        async def process_batch(batch: list) -> Any:
            async with semaphore:
                return await asyncio.wait_for(processor_func(batch), timeout=self.config.batch_timeout)
