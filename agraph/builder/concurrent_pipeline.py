...
class ConcurrentPipeline(BuildPipeline):
    ...
    def _build_parallel_groups(self) -> None:
    ...
    def _topological_sort(self) -> List[str]:
        ...
        in_degree = {name: 0 for name in self._step_dependencies}
        adjacency: Dict[str, List[str]] = {name: [] for name in self._step_dependencies}
    ...
    async def execute(self, context: BuildContext) -> KnowledgeGraph:
        ...
        try:
            logger.info(
                f"Starting concurrent pipeline with {len(self.steps)} steps in {len(self._parallel_groups)} groups"
            )
            ...
            if not knowledge_graph:
                raise RuntimeError("Pipeline completed but no knowledge graph was created")
            ...
            logger.info(
                f"Final graph: '{knowledge_graph.name}' with {len(knowledge_graph.entities)} entities, "
    ...
    async def _execute_single_step(self, step: BuildStep, context: BuildContext) -> None:
        ...
        if result.is_success():
            context.mark_step_completed(step.name, step_end_time, result.data, result.metadata)
    ...
    async def _execute_parallel_group(self, steps: List[BuildStep], context: BuildContext) -> None:
        """Execute a group of steps in parallel."""
        logger.info(f"Executing {len(steps)} steps in parallel: {[s.name for s in steps]}")

        async def execute_step_with_context(step: BuildStep) -> None:
        ...
        results = await asyncio.gather(*[execute_step_with_context(step) for step in steps], return_exceptions=True)
    ...
    def get_execution_plan(self) -> Dict[str, Any]:
        ...
        for i, group in enumerate(self._parallel_groups):
            group_info = {
                ...
                "estimated_concurrency": min(
                    len(group),
                    max(1, len([s for s in group if self._step_dependencies[s].can_run_in_parallel])),
                ),
            }
